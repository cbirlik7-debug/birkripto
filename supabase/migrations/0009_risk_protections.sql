alter table bot_config add column if not exists max_daily_loss_pct numeric not null default 5;
alter table bot_config add column if not exists max_consecutive_losses int not null default 3;
alter table bot_config add column if not exists max_open_positions int not null default 1;
alter table bot_config add column if not exists cooldown_minutes int not null default 0;
...
begin
  ...
  set
    ...
    max_daily_loss_pct = coalesce((p_changes->>'max_daily_loss_pct')::numeric, max_daily_loss_pct),
    max_consecutive_losses = coalesce((p_changes->>'max_consecutive_losses')::int, max_consecutive_losses),
    max_open_positions = coalesce((p_changes->>'max_open_positions')::int, max_open_positions),
    cooldown_minutes = coalesce((p_changes->>'cooldown_minutes')::int, cooldown_minutes)
  ...
  if updated_config.min_confluence_score not between 1 and 4
    ...
    or updated_config.leverage not between 1 and 20
    or updated_config.max_daily_loss_pct < 0
    or updated_config.max_daily_loss_pct > 100
    or updated_config.max_consecutive_losses < 0
    or updated_config.max_consecutive_losses > 100
    or updated_config.max_open_positions < 1
    or updated_config.max_open_positions > 20
    or updated_config.cooldown_minutes < 0
    or updated_config.cooldown_minutes > 10080 then
